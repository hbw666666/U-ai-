...
2. Put the provided `lib/` folder next to the jar (the jar's manifest
   references `lib/*.jar`, so the dependencies must stay in `lib/`).
...
`lib/` (the third-party dependencies) is **not** included; copy it from
the original 1.0.3 release. The patched jar keeps the original manifest,
so `lib/` must be present for the program to start.
